<?php
// $Id: comment_new.php,v 1.1 2004/12/06 18:13:29 robekras Exp $
//  ------------------------------------------------------------------------ //
...
$com_itemid = isset($_GET['com_itemid']) ? (int)$_GET['com_itemid'] : 1;
...
if ($com_itemid > 0) {
    ...
    if (isset($com_replytitle)) {
        ...
        $com_title = htmlspecialchars($com_replytitle, ENT_QUOTES | ENT_HTML5);

        if (!preg_match('/^re:/i', $com_title)) {
            $com_title = 'Re: ' . mb_substr($com_title, 0, 56);
        }
    } else {
        $com_title = '';
    }
